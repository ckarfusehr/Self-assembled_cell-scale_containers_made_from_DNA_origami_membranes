...
def read_sim_setup():
	with open('sim_setup.yml', 'r') as f:
		config = yaml.safe_load(f)
	use_force_file = bool(config['use_force_file'])
	do_initial_relaxation = bool(config['do_initial_relaxation'])
	continue_with_sim = bool(config['do_production_sim'])

	return use_force_file, continue_with_sim, do_initial_relaxation
...
def auto_detect_files():
	...
	for file in files_in_dir:
		print(f"File going trhough autodetection: {file}")
		if file[-4:] == ".top":
			topology = file
		elif (".dat" in file or ".conf" in file) and "trj" not in file:
			initial_conf = file
		elif (".frc" in file) or ("force" in file):
			forces = file
...
def main():
	topology, initial_conf, forces = auto_detect_files()
	use_force_file, continue_with_sim, do_initial_relaxation = read_sim_setup()
	print(f"Alright, let shit hit the fan")
	if do_initial_relaxation:
		print(f"detected top file: {topology}, detected conf file: {initial_conf}, detected force file: {forces}")
